...
> thirty-one automated tests covering every test case in our report."
...
| 14 | Run `python -m pytest tests/ -v` | "All thirty-one tests pass." |
...
No — that is a known limitation we have documented. A malicious page could in
principle submit a form on behalf of a logged-in user. The standard fix in
Flask is Flask-WTF's `CSRFProtect`, which adds a hidden token to every form
and rejects requests without it. We would add that before deploying publicly.
...
in the `tests/` folder — thirty-one tests, one file per area.
...
- **The secret key is in the source** — for a real deployment it belongs in an
  environment variable, like the database URL already does.
...
| Stack | Python, Flask, SQLAlchemy ORM, PyMySQL, MySQL, Bootstrap 5, Chart.js |
